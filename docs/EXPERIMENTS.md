...
`grid-search` выполняет реальные workunit: подготовительный шаг на сервере
создаёт общий CSV-датасет, а каждая workunit получает этот CSV как входной файл
BOINC и считает ridge-регрессию для одного значения `lambda`. Количество
workunit, размер датасета и параметры сетки заданы в
`apps/ml_grid_search/main.py`. Сейчас используется `WORKUNITS=20`;
...
- `Полезная нагрузка` — доля compute-time первой успешной attempt на workunit; реплики и ошибки считаются накладными расходами.
...
Отказ считается детерминированно по `seed + task_id + hostname + result name`,
поэтому replacement attempt может отличаться от первой попытки даже на том же
узле. Это лучше, чем специально ломать задачу по `task_id`: такая ошибка
повторится во всех репликах и quorum не сможет восстановиться.
