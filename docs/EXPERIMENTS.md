...
- `Полезная нагрузка` — доля выполненного compute-time, потраченная на первые `min_quorum` успешных attempts; лишние реплики и ошибки считаются неполезным compute-time.
