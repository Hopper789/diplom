...
- `Полезная нагрузка` — доля полезного compute-time с учётом реальных накладных расходов; локальное ожидание заранее выданных задач на клиенте вычитается, лишние реплики и ошибки снижают показатель;
