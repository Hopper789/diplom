...
- `Полезная нагрузка` — доля вычислительного времени в жизненном цикле первых `min_quorum` успешных attempts; нормальная репликация учитывается отдельно в `Факт. репликация`;
