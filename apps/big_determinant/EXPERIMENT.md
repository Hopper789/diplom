...
- повторяет расчёт до `target_seconds`;
- возвращает знак определителя, `log_abs`, научную форму и время выполнения.

По умолчанию одна workunit работает примерно 30 минут:
...
```bash
DETERMINANT_TASK_SECONDS=1 DETERMINANT_TASK_COUNT=1 DETERMINANT_MATRIX_SIZE=32 \
...
| Переменная | Смысл |
|---|---|
| `DETERMINANT_TASK_SECONDS` | целевая длительность одной workunit в секундах |
